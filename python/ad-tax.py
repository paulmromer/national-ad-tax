...
def table_marg_rates(b, r):
    ...
    for j in range(len(b)):
        ...
        t = r[j]
        if np.isnan(u):
            row_list.append(" " * 4 + f"Above {l:>3} billion {t:>17.1%}" + " " * 8)
        else:
            row_list.append(" " * 5 + f" {l:>2} and {u:>2} billion {t:>17.1%}" + " " * 8)

    return h_table(row_list, font_size=12, display_html = False, return_html = True)
...
def table_tax_paid_by_firm(b, r):
    ...
    return h_table(row_list, display_html = False, return_html = True)
...
def table_tax_paid_total(b, r):
    ...
    return h_table(row_list, header_rows = 2, font_size=12, display_html = False, return_html = True)    
